...
DOCKER_IMAGE=nvcr.io/nvidia/clara/monai-toolkit:1.0
echo "Starting docker with $DOCKER_IMAGE"
docker run --rm -it --name=site-6 --gpus=$GPU2USE -u $(id -u):$(id -g) -v /etc/passwd:/etc/passwd -v /etc/group:/etc/group -v $DIR/..:/workspace/ -v $MY_DATA_DIR:/data/:ro -w /workspace/ --ipc=host $NETARG $DOCKER_IMAGE /bin/bash
